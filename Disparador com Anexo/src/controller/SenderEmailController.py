...
class SenderEmailController(SenderEmailModel):
    ...
    def load_attachments(list_attachments: list[str]) -> list[MIMEApplication] | None:
        ...
        try:
            for attachment_name in list_attachments:
                attachment_path: str = os.getcwd(
                ) + f"\\anexos\\{attachment_name}"
                with open(attachment_path, 'rb') as file:
                    part: MIMEApplication = MIMEApplication(
                        file.read(), Name=os.path.basename(attachment_name))
            ...
            raise PermissionError(
                "Permissão negada para abrir o arquivo. Verifique se o arquivo está em uso.") from pe
        except IsADirectoryError as ide:
            raise IsADirectoryError(
                "O caminho é um diretório, não um arquivo.") from ide
        except OSError as ose:
            raise OSError(
                f"Ocorreu um erro ao tentar abrir o arquivo: {ose.strerror}") from ose
    ...
    def __init__(self, data_user: DataUserController, spreadsheet_path: str, html_path: str, email_send_quantity: int, send_interval: int, email_subject: str, email_title: str, email_message: str, whatsapp_redirect_number: str):
        super().__init__(
            ...
            list_columns=['E-mail', 'Nome',
                          'Protocolo', 'Produto', 'PDF', 'STATUS']
        )
    ...
    def read_emails(self) -> tuple[list[str], pd.DataFrame]:
        ...
        try:
            ...
            missing_columns: list[str] = list(
                set(required_columns) - set(df_emails.columns))
            raise KeyError(
                f"Colunas ausentes na planilha: {', '.join(missing_columns)}") from error
    ...
    def get_name_product(self, df_emails: pd.DataFrame, destination_email: str) -> tuple[str, str, str]:
        if destination_email in df_emails[self.list_columns[0]].values:
            name: str = df_emails.loc[df_emails[self.list_columns[0]]
                                      == destination_email, self.list_columns[1]].iloc[0]
            product: str = df_emails.loc[df_emails[self.list_columns[0]]
                                         == destination_email, self.list_columns[2]].iloc[0]
            protocol: str = df_emails.loc[df_emails[self.list_columns[0]]
                                          == destination_email, self.list_columns[3]].iloc[0]
    ...
    def get_name_attachment(self, df_emails: pd.DataFrame, destination_email: str) -> list[str] | list:
        if destination_email in df_emails[self.list_columns[0]].values:
            attachments: str = df_emails.loc[df_emails[self.list_columns[0]]
                                             == destination_email, self.list_columns[4]].iloc[0]
            ...
            if ";" in attachments:
                arquives_name_list.extend(attachments.split(';'))
            else:
                arquives_name_list.append(attachments)
    ...
    def type_verify_email(self) -> str:
        ...
        return email_and_server[domain] if domain in email_and_server else email_and_server["smtp.gmail.com"]
    ...
    def replace_email_html(self, body_html_original: str, message_copy: str, title_copy: str, link_whatsapp_copy: str) -> str:
        ...
        copy_body_html = copy_body_html.replace(
            '{mensagem_html}', message_copy)
        copy_body_html = copy_body_html.replace(
            '{link_whatsapp}', link_whatsapp_copy)
    ...
    def send_emails(self, log_signal: pyqtBoundSignal) -> None:
        ...
        for i, destination_email in enumerate(destination_emails[start_index:]):
            ...
            try:
                name, product, protocol = self.get_name_product(
                    df_emails, destination_email)
                ...
                message_copy, title_copy, link_whatsapp_copy = self.update_message(
                    name, product, protocol)
                copy_body_html: str = self.replace_email_html(
                    body_html_original, message_copy, title_copy, link_whatsapp_copy)
                self.config_send_email(
                    destination_email, copy_body_html, attachments_list_in_memory)
                random_interval: int = SenderEmailController.wait_random_send_email(
                    self.send_interval)


                df_emails.loc[i, self.list_columns[5]] = 'ENVIADO'
                df_emails.to_excel(self.spreadsheet_path,
                                   index=False)
                message_success: str = self.write_on_console_and_txt(
                    destination_email, destination_emails, True, i, start_index, None, random_interval)
                ...
                df_emails.to_excel(self.spreadsheet_path,
                                   index=False)
                message_error: str = self.write_on_console_and_txt(
                    destination_email, destination_emails, False, i, None, error, None)
